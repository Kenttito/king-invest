...
const { sendVerificationEmail } = require('./test-email-verification');
...
async function generateCodeForUser(email) {
  try {
    ...
    if (emailSent) {
      ...
      console.log(`⚠️  Email sending failed, but code was generated: ${verificationCode}`);
    }
...
async function main() {
  ...
  if (!email) {
    ...
    console.log('Usage: node generate-code-for-user.js <email>');
    console.log('');
  ...
  if (code) {
    ...
    console.log('Use this code to verify the user account');
  }
